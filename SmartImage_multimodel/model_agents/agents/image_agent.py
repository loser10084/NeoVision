# model_agents/agents/image_agent.py
...
def create_image_agent(llm):
    ...
    """
    # 读取图像系统提示词
    image_system_prompt_path = Path(__file__).parent.parent / "image_system_prompt.md"
    with open(image_system_prompt_path, 'r', encoding='utf-8') as f:
    ...
    def image_agent_node(state):
        ...
        # 执行智能体
        logger.debug(f"开始执行 [image_agent_node] input={state['messages']}")
        result = chain.invoke(state["messages"])
        logger.debug(f"[image_agent_node] result={result}") 
        ...
        # 更新状态
        return {
            "messages": state["messages"] + [result], # merge_messages 会处理追加
            "sender": "image_agent", # 声明身份
            "step_count": state.get("step_count", 0) + 1, # 自动累加
            "search_count": current_search_count + search_tool_usage,  # 累加搜索次数
            "consecutive_search_count": current_consecutive_search_count + search_tool_usage  # 累加连续搜索次数
        }
